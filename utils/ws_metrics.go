...
// WSMetricsCollector collects WebSocket connection metrics
...
// NewWSMetricsCollector creates a new WebSocket metrics collector
func NewWSMetricsCollector() *WSMetricsCollector {
	return &WSMetricsCollector{
...
func (w *WSMetricsCollector) RecordMessageReceived(bytes int, latency time.Duration) {
...
func (w *WSMetricsCollector) RecordMessageSent(bytes int) {
	w.messagesSent.Add(1)
	w.bytesSent.Add(int64(bytes))
}

// RecordError records an error
func (w *WSMetricsCollector) RecordError() {
	w.errors.Add(1)
}

// RecordConnection records a connection state change
func (w *WSMetricsCollector) RecordConnection(connected bool) {
	if connected {
...
func (w *WSMetricsCollector) RecordReconnection() {
	w.reconnections.Add(1)
}

func (w *WSMetricsCollector) recordLatency(latency time.Duration) {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *WSMetricsCollector) GetMetrics() map[string]interface{} {
	w.mu.RLock()
	defer w.mu.RUnlock()
...
func (w *WSMetricsCollector) Reset() {
